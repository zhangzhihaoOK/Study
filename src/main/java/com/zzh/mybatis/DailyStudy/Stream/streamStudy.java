...
public class streamStudy {
    public static void main(String[] args) {
        ...
        List<Personal> plist = new ArrayList<>();

        plist.add(new Personal("张三",26,7800));
        plist.add(new Personal("李四",80,8000));
        plist.add(new Personal("王五",22,7600));
        plist.add(new Personal("赵六",30,8100));
        plist.add(new Personal("孙七",18,8900));
        //工资高于7850的员工姓名
        List<String> collect = plist.stream().filter(x -> x.getSalary() > 7850).map(Personal::getName).collect(Collectors.toList());
        System.out.println("工资高于7850的员工姓名  " + collect);
        //年龄大于28岁的员工姓名
        List<String> collect1 = plist.stream().filter(x -> x.getAge() > 28).map(Personal::getName).collect(Collectors.toList());
        System.out.println("年龄大于28岁的员工姓名  " + collect1);
        //员工工资的最大值
        Optional<Personal> maxSl = plist.stream().max(Comparator.comparingInt(Personal::getSalary));
...
//        intlist.stream().map(x -> x + 3).forEach(System.out::println);
        ...
        //将员工的薪资全部增加1000。
        // 不改变原来员工集合的方式
        List<Personal> PerAddSa = plist.stream().map(Personal -> {
                    Personal PersonalNew = new Personal(Personal.getName(), 0, 0);
                    PersonalNew.setSalary(Personal.getSalary() + 1000);
                    return PersonalNew;
                }
...
//        List<Personal> PerAddSA = plist.stream().map(Personal -> {
//            Personal PersonalNew = new Personal(Personal.getName(), 0, 0);
//            Personal.setSalary(Personal.getSalary() + 1000);
//            return PersonalNew;
//        }).collect(Collectors.toList());
//        for (int i = 0; i < 5; i++) {
...
//        求Integer集合的元素之和、乘积和最大值。
        ...
        //求所有员工的工资之和
        Optional<Integer> sumSA = plist.stream().map(Personal::getSalary).reduce(Integer::sum);
...
//        plist.add(new Personal("孙七",18,8900));
        //将工资大于8000的分到一个集合中
        Map<?,Integer> perMap = plist.stream().filter(p -> p.getSalary() > 8000).collect(Collectors.toMap(Personal::getName,p -> p.getSalary()));
        ...
        //求平均工资
        Double AvgSa = plist.stream().collect(Collectors.averagingDouble(Personal::getSalary));
        System.out.println("平均工资为  " + AvgSa);

        //求最高工资
        Optional<Integer> maxSA = plist.stream().map(Personal::getSalary).collect(Collectors.maxBy(Integer::compareTo));
        System.out.println("最高工资为  " + maxSA.get());

        //求工资之和
        Integer sumSa = plist.stream().collect(Collectors.summingInt(Personal::getSalary));
        System.out.println("工资之和为  " + sumSa);

        // 一次性统计信息  有关工资的最大最小值 平均值 综合 个数
        DoubleSummaryStatistics all = plist.stream().collect(Collectors.summarizingDouble(Personal::getSalary));
...
//        员工按薪资是否高于8000分为两部分
        ...
        Map<Boolean, List<Personal>> isUp8 = plist.stream().collect(Collectors.partitioningBy(x -> x.getSalary() > 8000));
...
//        isUp8.forEach((k,v) ->System.out.println( k + " " + v.stream().map(p -> p.getName()).collect(Collectors.toList())));//false [张三, 李四, 王五]  true [赵六, 孙七]
        ...
        Map<Boolean, List<Personal>> isUp25 = plist.stream().collect(Collectors.groupingBy(x -> x.getAge() > 25));
        isUp25.forEach((k,v) -> System.out.println(k + "  " + v.stream().map(Personal::getName).collect(Collectors.joining(","))));

        // 将员工先按年龄分组，再按工资分组
        Map<Boolean, Map<Boolean, List<Personal>>> AgeSa = plist.stream().collect(Collectors.partitioningBy(p -> p.getAge() > 25, Collectors.groupingBy(p -> p.getSalary() > 8000)));
        System.out.println(AgeSa);

        //归约(reducing)
        //Collectors类提供的reducing方法，相比于stream本身的reduce方法，增加了对自定义归约的支持。
        //每个员工减去起征点后的薪资之和  起征点为5000
        Integer sa5k = plist.stream().collect(Collectors.reducing(0, Personal::getSalary, (x, y) -> (x + y - 5000)));
        //Integer sa5k = plist.stream().map(Personal::getSalary).reduce(0, (x, y) -> (x + y - 5000));
        ...
        List<String> SaSort = plist.stream().sorted(Comparator.comparing(Personal::getSalary)).map(Personal::getName).collect(Collectors.toList());
        System.out.println("按工资升序排序：" +SaSort);
        //将员工按工资由高到低排序
        List<String> reSaSort = plist.stream().sorted(Comparator.comparing(Personal::getSalary).reversed()).map(Personal::getName).collect(Collectors.toList());
        System.out.println("按工资降序排序：" +reSaSort);

        List<Personal> pplist = new ArrayList<>();
        pplist.add(new Personal("张三",26,7800));
        pplist.add(new Personal("李四",80,8000));
        pplist.add(new Personal("王五",22,7600));
        pplist.add(new Personal("赵六",30,8100));
        pplist.add(new Personal("孙七",18,8900));
        pplist.add(new Personal("小二",20,7800));
        pplist.add(new Personal("周九",21,7600));
        // 先按工资再按年龄升序排序
        List<String> SaAge = pplist.stream().sorted(Comparator.comparing(Personal::getSalary).thenComparing(Personal::getAge)).map(Personal::getName).collect(Collectors.toList());
...
//            }
//        }).map(Personal::getName).collect(Collectors.toList());
...
class Personal{
    ...
    public Personal(String name, int age, int salary) {
        this.name = name;
        this.age = age;
        this.salary = salary;
    }
